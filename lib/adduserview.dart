...
class StaffSavingView extends HookConsumerWidget {
  const StaffSavingView({super.key, required this.newUser});
      ...
      body: SafeArea(
        ...
        child: Column(
          ...
          children: [
            ...
            ref.watch(saveToDB(newUser)).when(
                data: (data) {
                  return SizedBox(
                    height: 150,
                    child: Column(
...
final _savingStatus = StateProvider.autoDispose((ref) => 0);

final saveToDB =
    FutureProvider.autoDispose.family<bool, UserModel>((ref, newUser) async {
  final getIt = GetIt.instance;
  ref.watch(_savingStatus.notifier).state = 0;
  await Future.delayed(const Duration(seconds: 4));

  await getIt<DatabaseHandler>().addUpdateNewUser(newUser);
